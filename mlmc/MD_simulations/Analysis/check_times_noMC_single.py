...
def create_figures_0(dir:str, block=True):    
    ...
    for d in data:
        ...
        plt.plot(c_0_vec, tol_0_vec, '-s', label=d[0], linewidth=3, markersize=24)
...
def create_figures_1(dir:str, block=True):
    ...
    for d in data:
        ...
        plt.plot(time_0_vec, tol_0_vec, '-s', label=d[0], linewidth=3, markersize=24)

        max_time_0 = max(max_time_0, max(time_0_vec))

    ax = fig.get_axes()[0]    
    ax.plot( (0, max_time_0), (5e-5, 5e-5), '--', label='$\epsilon=5e-5$')
    ax.set_yscale('log')
    ax.set_xscale('log')
